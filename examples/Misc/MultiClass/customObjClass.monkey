...
Class cCustomObj Extends ftObject
	...
	Method Render:Void(xoff:Float=0.0, yoff:Float=0.0)
		...
		Self.engine.GetCanvas().PushMatrix
		Self.engine.GetCanvas().Translate ((xPos+xoff), (yPos+yoff)) 
		Self.engine.GetCanvas().Rotate 360.0-angle
		Self.engine.GetCanvas().Scale (Self.scaleX, Self.scaleY)
		Self.engine.GetCanvas().DrawCircle -Self.w * (Self.handleX-0.5), -Self.h * (Self.handleY-0.5), Self.radius
		Self.engine.SetColor(255,255,0)
		Self.engine.SetAlpha(1.0)
		Self.engine.GetCanvas().DrawRect -5, -h*Self.handleY, 10, h
		...
		Self.engine.GetCanvas().PopMatrix
